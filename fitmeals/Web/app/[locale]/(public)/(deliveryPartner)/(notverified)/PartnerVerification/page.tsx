...
import {
  isRiderUnderVerification,
  makeRiderVerified,
} from "@/app/api/actions/Rider/verification";
...
export default function PartnerVerificationPage() {
  ...
  useEffect(() => {
    const checkVerification = async () => {
      if (userId) {
        try {
          const status = await isRiderUnderVerification(userId);
  ...
  const handleVerified = async () => {
    ...
    try {
      setLoading(true);
      await makeRiderVerified(userId);
      toast.success(t("successVerification"));
      globalThis.location.href = `/${locale}/dashboard/deliveryPartner`;
    } catch (err) {
      console.log(err);
      ...
      <div className="min-h-screen bg-[#fafafa] relative overflow-hidden flex items-center justify-center font-manrope px-4 py-12">
        ...
        >
          ...
          <motion.div variants={itemVariants}>
            <button
              onClick={() =>
                (globalThis.location.href = `/${locale}/form/deliveryPartner`)
              }
  ...
  return (
    <div className="min-h-screen bg-[#fafafa] relative overflow-hidden flex items-center justify-center font-manrope px-4 py-12">
      ...
      >
        ...
        >
          <div className="flex-1 max-w-[220px] mx-auto sm:mx-0">
            <button
              disabled={loading}
